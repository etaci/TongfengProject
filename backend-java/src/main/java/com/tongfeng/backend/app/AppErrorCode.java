...
public enum AppErrorCode {
	...
	CLINICAL_DECISION_PAYLOAD_MISSING("CLINICAL_DECISION_PAYLOAD_MISSING", HttpStatus.INTERNAL_SERVER_ERROR, "CLINICAL", false, "临床决策可信载荷缺失"),
	DOCTOR_SHARE_NOT_FOUND("DOCTOR_SHARE_NOT_FOUND", HttpStatus.NOT_FOUND, "SHARING", false, "就诊包分享链接不存在"),
	DOCTOR_SHARE_EXPIRED("DOCTOR_SHARE_EXPIRED", HttpStatus.GONE, "SHARING", false, "就诊包分享链接已过期或撤销"),
	DOCTOR_PACKAGE_PDF_FAILED("DOCTOR_PACKAGE_PDF_FAILED", HttpStatus.INTERNAL_SERVER_ERROR, "DOCUMENT", true, "医生就诊包 PDF 生成失败"),
	DATA_EXPORT_FAILED("DATA_EXPORT_FAILED", HttpStatus.INTERNAL_SERVER_ERROR, "PRIVACY", true, "个人数据导出失败"),
	ACCOUNT_DELETE_CONFIRMATION_INVALID("ACCOUNT_DELETE_CONFIRMATION_INVALID", HttpStatus.BAD_REQUEST, "PRIVACY", false, "账号删除确认文本不正确"),
